...
describe("Rate Limiting", () => {
  ...
  describe("RATE_LIMITS presets", () => {
    ...
    it("has correct BROADCAST_TEST limits", () => {
      expect(RATE_LIMITS.BROADCAST_TEST.limit).toBe(10);
      expect(RATE_LIMITS.BROADCAST_TEST.windowSeconds).toBe(600);
    });
  });

  describe("verifyAdminPassword", () => {
    it("matches the configured admin password", () => {
      const previous = process.env.ADMIN_PASSWORD;
      process.env.ADMIN_PASSWORD = "supersecret";

      expect(verifyAdminPassword("supersecret")).toBe(true);
      expect(verifyAdminPassword("wrongsecret")).toBe(false);
